...
Write-Host "=== Taller #3: verificación completa SQLite limpia ===" -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Falló la auditoría estática." }

    python manage.py check
    if ($LASTEXITCODE -ne 0) { throw "Falló manage.py check." }

    python manage.py makemigrations --check --dry-run
    if ($LASTEXITCODE -ne 0) { throw "Hay cambios de modelos sin migración." }

    python manage.py migrate --noinput
    if ($LASTEXITCODE -ne 0) { throw "Falló la migración limpia." }

    python manage.py showmigrations
    if ($LASTEXITCODE -ne 0) { throw "Falló showmigrations." }

    python manage.py seed_baseline
    if ($LASTEXITCODE -ne 0) { throw "Falló seed_baseline." }

    python scripts/smoke_runserver.py
    if ($LASTEXITCODE -ne 0) { throw "La landing no respondió con runserver." }

    python manage.py test --verbosity 2
    if ($LASTEXITCODE -ne 0) { throw "Falló la suite Django." }

    python manage.py findstatic css/app.css js/app.js img/logo-placeholder.png
    if ($LASTEXITCODE -ne 0) { throw "No se encontraron los recursos static." }
    ...
    if (Test-Path (Join-Path $ProjectRoot "staticfiles")) {
        Remove-Item (Join-Path $ProjectRoot "staticfiles") -Recurse -Force
    }
