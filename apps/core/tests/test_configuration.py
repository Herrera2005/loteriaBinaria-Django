...
class ConfigurationTests(SimpleTestCase):
    ...
    def test_relative_sqlite_url_resolves_under_project_root(self):
        ...
        environment.update(
            {
                "DATABASE_URL": "sqlite:///verification.sqlite3",
                "DJANGO_DEBUG": "True",
            }
        )
        ...
        command = (
            "from pathlib import Path; "
            "from django.conf import settings; "
            "database_name = Path(str(settings.DATABASES['default']['NAME'])); "
            "print(database_name.is_absolute()); "
            "print(database_name.parent == Path(settings.BASE_DIR))"
        )
        ...
        self.assertEqual(output_lines[-2:], ["True", "True"])
    ...
    def test_postgresql_database_url_is_rejected_for_this_workshop(self):
        ...
        environment.update(
            {
                "DATABASE_URL": (
                    "postgresql://user:password@127.0.0.1:5432/forbidden"
                ),
                "DJANGO_DEBUG": "True",
            }
        )
        ...
        self.assertIn(
            "únicamente SQLite en fase 1 y MySQL en fase 2",
            combined_output,
        )
